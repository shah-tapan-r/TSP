...
class TSPTWHegel   //MINIMIZES!!!
{
    ...
    void newRestart(void)
   {
      ...
      if (tourEnd<n-1)
	{
	  ...
	  int negSize=myRand(1,maximum(negSpace,1)); //myRand(maximum(minimum(negSpace,(int)round(n*minPercent*0.01)),1),maximum(minimum((int)round(n*maxPercent*0.01),negSpace),1));
    ...
    bool eavesdrop(bool surrogate=false)
    {
      char* filename=collabInFile;
      if (surrogate) filename=inFile;
...
public:
 ...
 TSPTWHegel(int _n, Variable*& v, Eval* _e, int _sSeeds, double _redScenStart, double _redScenEnd, LearnParam* _gV, LearnParam* _aG, LearnParam* _aMin, LearnParam* _aMax, 
       ...
       : n(_n), e(_e), redScenStart(_redScenStart), redScenEnd(_redScenEnd), redScenFac(_redScenStart)
	...
	std::cout << perm[i] << " ";
      ...
      */
      ...
      remove (collabOutFile);
        ...
        //mySRand(212329);
        for (int i=0; i<n; i++)
        thesis[i]=e->getValue(i); //(bool)myRand(0,1);
	/*
        for (int i=0; i<n; i++)
	  std::cout << thesis[i] << " ";
	std::cout << endl;
