...
#ifdef MINOROBJ
	    nodeTimePenalty+=minorTimePenalty[ti]*currentArrivalDistribution[ti];
#endif
	    currentArrivalDistribution[ti]=0;
	  }
	startMass+=currentArrivalDistribution[nodeStart[currentNode]];
#ifdef MINOROBJ
	if (nodeStart[currentNode]<=maxTime) nodeTimePenalty+=minorTimePenalty[nodeStart[currentNode]]*currentArrivalDistribution[nodeStart[currentNode]];
#endif
	...
	for (int ti=nodeStart[currentNode]+1; ti<=nodeEnd[currentNode]; ti++)
	  {
	    startMass+=currentArrivalDistribution[ti];
#ifdef MINOROBJ
	    if (ti<=maxTime) nodeTimePenalty+=minorTimePenalty[ti]*currentArrivalDistribution[ti];
#endif
	    /*
#ifdef MINOROBJ
	    fac-=facdec;
	    nodeTimePenalty+=fac*currentArrivalDistribution[ti];
	    probfacmass+=fac;
#endif
	    */
	  }
...
public:
  ...
  TSPTW(int _n, int _k, char* _nodeFile, char* _arcFile, double _nodePenalty, int _maxTime, double _tourPenalty, double _polyExp) : n(_n), k(_k), nodePenalty(_nodePenalty), tourPenalty(_tourPenalty), 
    kChange(true)
#ifdef MINOROBJ
																  , minorCoeff(0.01), minorPolyExp(_polyExp)
#endif
  {
    nodeStart=new int[n];
    nodeEnd=new int[n];
...
int main(int argc, char** argv)
{
  ...
  TSPTW tsp(n,k,argv[3],argv[4],nodePen,solTimeLimit,tourPen,polyExp);
  ...
  TSPTWHegel hegel(tsp.getNumberOfNodes(), v, &tsp, startSeeds, 0.04, 0.06, gV, aG, aMin, aMax, alMin, alMax, alProb, aRProb, nI, rMin, rMax);
  delete [] params;
