...
class TSPTWHegel   //MINIMIZES!!!
{
    ...
    void newRestart(void)
   {
    ...
    bool greedyCore(double percentVars, bool anti, bool full, bool useCurrent=false, int must=-1) 
      ...
      if (!useCurrent)
	{
	  ...
	  int maxSelect=n-1; //n-1;//minimum(n,currentInv[0]+1); //n; //minimum(n,currentInv[0]+5);
	  ...
	  */
	  bin.select(true);
	  //if (useCurrent) bin[0]=must;
	  SSet selected(maxSelect+1);
	  for (int s=0; s<h; s++)
            {
	      int i=bin[s];
	      assert(i>=0 && i<n);
	      if (useCurrent&&s==0) i=must;
	      assert(i>=0 && i<n);
    ...
    void throwAntithesisSet(double minPercent, double maxPercent, bool det=false)
    {
      copyThesisToCurrent();
      ...
      if (tourEnd<n-1)
	{
	  ...
	  int negSize;
	  if (!det) negSize=myRand(1,maximum(negSpace,1)); //negSpace; //myRand(maximum(minimum(negSpace,(int)round(n*minPercent*0.01)),1),maximum(minimum((int)round(n*maxPercent*0.01),negSpace),1)); 
	  else negSize=maximum(negSpace,1);
//myRand(1,maximum(negSpace,1)); //myRand(maximum(minimum(negSpace,(int)round(n*minPercent*0.01)),1),maximum(minimum((int)round(n*maxPercent*0.01),negSpace),1));
	  ...
	  for (int i=0; i<negSize; i++)
	    {
	      ...
	      if (tourEnd+1+bin[i]>=0 && tourEnd+1+bin[i]<n && current[tourEnd+1+bin[i]] >=0 &&  current[tourEnd+1+bin[i]] <n)
		{
		  if (!det || e->nodeStart[current[tourEnd+1+bin[i]]]<=e->maxTime) 
		    antiNegSet.add(current[tourEnd+1+bin[i]]);
		  assert(currentInv[current[tourEnd+1+bin[i]]]==tourEnd+1+bin[i]);
		}
	    }
	}
      //std::cout << std::endl;
	  ...
	  if (current[s]==0)
	    {
	      assert(tempEnd<0);
	      tempEnd=s;
	      assert(tempEnd==currentInv[0]);
	    }
	}
      for (int ih=0; ih<antiNegSet.card-1; ih++)
	{
	  int i=antiNegSet.list[ih];
	  assert(currentInv[i]>tempEnd);
	  int h=currentInv[i];
	  for (int jh=0; jh<antiPosSet.card; jh++)
	    {
	      int j=antiPosSet.list[jh];
	      if (j==i) continue;
	      //std::cout << "(" << i << "," << j << ") ";
	      for (int kh=ih+1; kh<antiNegSet.card; kh++)
		{
		  int k=antiNegSet.list[kh];
		  if (i==k) continue;
		  current[h]=j;
		  current[currentInv[j]]=i;
		  currentInv[i]=currentInv[j];
		  currentInv[j]=h;
		  assert(currentInv[k]>tempEnd);
		  assert(tempEnd<n-2);
		  current[tempEnd]=k;
		  current[currentInv[k]]=current[tempEnd+1];
		  currentInv[current[tempEnd+1]]=currentInv[k];
		  currentInv[k]=tempEnd;
		  current[tempEnd+1]=0;
		  ...
		  printVector(current);
		  */
		  TSPTWFIX fix(tempEnd+1,current,n,variables,e,greedyVars, antiGreedy, antiMinSize, antiMaxSize, noImprovementRestart, restartMinSize, restartMaxSize);		  
		  double qualy = fix.minimize(0.05,EPSILON);
		  //	      std::cout <<  "qualy is " << qualy << std::endl;
		  if (qualy<0) 
		    {
		      //std::cout << "YESSS: ";
		      fix.recordBest(&current[0]);
		      //printVector(current);
		      assert(current[tempEnd+1]==0);
		      moveThesisToCurrent();
		      thesisVal=qualy;
		  ...
		  for (int s=0; s<n; s++)
		    {
		      current[s]=saved2[s];
		      currentInv[saved2[s]]=s;
		    }	      
		  //greedyCore(greedyVars->getPercentage(),false,false,true,i);
		}
	      /*
	      ...
	      if (currentVal<bestVal)
		{
	      ...
	      if (fabs(currentVal-thesisVal)<EPSILON)
		{
		  moved=true;
		  for (int s=0; s<n; s++)
		    {
      ...
      if (moved)
	{
	  if (bestVal<thesisVal)
	    {
	      for (int s=0; s<n; s++)
		{
		  thesis[s]=current[s]=saved3[s];
		}
	      thesisVal=currentVal=bestVal;
	      return 1;	      
	    }
	  for (int s=0; s<n; s++)
	    {
	      thesis[s]=current[s]=saved[s];
	    }
	  thesisVal=currentVal=e->eval(thesis);
	}
      //std::cout << "\ndone\n";
      ...
      while( getline(inf,temp) )
	{
      ...
      int selectedSeedIndex=candidates[myRand(0,numCands-1)]; //candidates[numCands-1];//candidates[numCands-1];//candidates[myRand(0,numCands-1)];
   ...
   : n(_n), e(_e), cleanE(_e2), redScenStart(_redScenStart), redScenEnd(_redScenEnd), redScenFac(_redScenStart)
      ...
      for (int rep=0; rep<0; rep++)
	{
	  ...
	  for (ind=0; ind<n; ind++)
	    {
	      t=perm[ind]+1;
	      if (e->nodeStart[t]<e->maxTime) break;
	    }
	  assert(t>=1 && t<n);
	  int pos=currentInv[t];
	  if (pos>cur)
	    {
	      current[pos]=current[cur];
	      current[cur]=t;
	      currentInv[t]=cur;
	      currentInv[current[pos]]=pos;
	    }
	  cur++;
	  t=0;
	  pos=currentInv[t];
	  ...
	  for (++ind; ind<n-1; ind++)
	    {
	      //std::cout << "cur -> " << cur << "  considering " << perm[ind]+1 << std::endl;
	      t=perm[ind]+1;
	      if (e->nodeStart[t]>=e->maxTime) continue;
	      
	      current[currentInv[t]]=current[cur];
	      currentInv[current[cur]]=currentInv[t];
	      current[cur]=t;
	      currentInv[t]=cur;
	      
	      current[currentInv[0]]=current[cur+1];
	      currentInv[current[cur+1]]=currentInv[0];
	      current[cur+1]=0;
	      currentInv[0]=cur+1;
	      //printVector(current);
	      TSPTWFIX fix(cur+1,current,n,variables,e,greedyVars, antiGreedy, antiMinSize, antiMaxSize, noImprovementRestart, restartMinSize, restartMaxSize);
	      
	      double qualy = fix.minimize(0.5,EPSILON);
	      //	      std::cout <<  "qualy is " << qualy << std::endl;
	      if (qualy<0) 
		{
		  int x[n];
		  ...
		  //printVector(x);
		  cur++; 
		  //std::cout << "construction improvement\n";
		  for (int j=0; j<cur; j++)
		    {
		      current[j]=x[j];
		      currentInv[x[j]]=j;
	  ...
	  if (!lastAdded)
	    {
	      ...
	      currentInv[current[cur]]=cur+1;
	      current[cur]=0;
	      currentInv[0]=cur;
	    }
	  double newVal=e->eval(current);
	  if (newVal<thesisVal)
	    {
	      moveThesisToCurrent();
	      thesisVal=newVal;
      ...
      /*
      for (int i=0; i<cur; i++) out.rem(thesis[i]);
      assert(out.card==n-1-cur);
      for (int i=cur+1; i<n; i++) thesis[i]=out.list[i-1-cur];
      */
      std::cout << "value after init -> " << thesisVal << std::endl;
      if (thesisVal<bestVal) 
	{
	  copyThesisToBest();
	  newBest();
